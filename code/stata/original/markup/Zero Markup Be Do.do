...
cd "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\Markup\be"			
			
log using "Thesis aggregated be Markup Calculation zero log", replace 			
*Import Data			
 use "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\Markup\be\Aggregated_be_zero.dta"			
...
foreach r of num 1(1)18 {			
    ...
    keep if sec == `r'			
	save acfest_data_be_zero_`r',replace		
...
*Dropping yearly losses becbee regression does not work with negative values			
			
	xtset id year, yearly		
forvalues r =  1(1)18 {			
	use acfest_data_be_zero_`r',replace		
	drop if pi < 0		
...
rename markup_acfest markup_be_zero			
xtline markup_be_zero, recast(line) tlabel(, labsize(medium) angle(vertical))			
gr export line_markup_be_zero_`r'.pdf,replace			
			
histogram markup_be_zero, kdensity kdenopts(lcolor(black) lwidth(thick) lpattern(solid) connect(direct))			
...
by size: gen delta_markup = (markup_be_zero - l.markup_be_zero) / l.markup_be_zero	*100		
			
save acfest_data_be_zero_`r',replace			
}			
			
use acfest_data_be_zero_1,clear			
			
forvalues r =  2/18{			
 append using acfest_data_be_zero_`r'.dta			
 }			
 egen grp = group (year sec)			
label var grp group_variable			
sort grp			
save be_markup_zero,replace			
save "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\HHI\Be\be_markup_zero.dta"			
