...
cd "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\Markup\pl"			
			
log using "Thesis aggregated pl Markup Calculation zero log", replace 			
*Import Data			
 use "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\Markup\pl\Aggregated_pl_zero.dta"			
...
foreach r of num 1(1)18 {			
    ...
    keep if sec == `r'			
	save acfest_data_pl_zero_`r',replace		
...
*Dropping yearly losses becple regression does not work with negative values			
			
	xtset id year, yearly		
forvalues r =  1(1)18 {			
	use acfest_data_pl_zero_`r',replace		
	drop if pi < 0		
...
rename markup_acfest markup_pl_zero			
xtline markup_pl_zero, recast(line) tlabel(, labsize(medium) angle(vertical))			
gr export line_markup_pl_zero_`r'.pdf,replace			
			
histogram markup_pl_zero, kdensity kdenopts(lcolor(black) lwidth(thick) lpattern(solid) connect(direct))			
...
by size: gen delta_markup = (markup_pl_zero - l.markup_pl_zero) / l.markup_pl_zero	*100		
			
save acfest_data_pl_zero_`r',replace			
}			
			
use acfest_data_pl_zero_1,clear			
			
forvalues r =  2/18{			
 append using acfest_data_pl_zero_`r'.dta			
 }			
 egen grp = group (year sec)			
label var grp group_variable			
sort grp			
save pl_markup_zero,replace			
save "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\HHI\Pl\pl_markup_zero.dta",replace	
