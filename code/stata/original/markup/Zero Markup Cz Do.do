...
cd "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\Markup\cz"			
			
log using "Thesis aggregated cz Markup Calculation zero log", replace 			
*Import Data			
 use "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\Markup\cz\Aggregated_cz_zero.dta"			
...
foreach r of num 1(1)18 {			
    ...
    keep if sec == `r'			
	save acfest_data_cz_zero_`r',replace		
...
*Dropping yearly losses beccze regression does not work with negative values			
			
	xtset id year, yearly		
forvalues r =  1(1)18 {			
	use acfest_data_cz_zero_`r',replace		
	drop if pi < 0		
...
rename markup_acfest markup_cz_zero			
xtline markup_cz_zero, recast(line) tlabel(, labsize(medium) angle(vertical))			
gr export line_markup_cz_zero_`r'.pdf,replace			
			
histogram markup_cz_zero, kdensity kdenopts(lcolor(black) lwidth(thick) lpattern(solid) connect(direct))			
...
by size: gen delta_markup = (markup_cz_zero - l.markup_cz_zero) / l.markup_cz_zero	*100		
			
save acfest_data_cz_zero_`r',replace			
}			
			
use acfest_data_cz_zero_1,clear			
			
forvalues r =  2/18{			
 append using acfest_data_cz_zero_`r'.dta			
 }			
 egen grp = group (year sec)			
label var grp group_variable			
sort grp			
save cz_markup_zero,replace			
save "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\HHI\Cz\cz_markup_zero.dta",replace	
