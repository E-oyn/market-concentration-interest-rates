...
cd "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\Markup\pt"			
			
log using "Thesis aggregated pt Markup Calculation zero log", replace 			
*Import Data			
 use "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\Markup\pt\Aggregated_pt_zero.dta"			
...
foreach r of num 1(1)18 {			
    ...
    keep if sec == `r'			
	save acfest_data_pt_zero_`r',replace		
...
*Dropping yearly losses becpte regression does not work with negative values			
			
	xtset id year, yearly		
forvalues r =  1(1)18 {		

capture {	
	use acfest_data_pt_zero_`r',replace		
	drop if pi < 0		
...
rename markup_acfest markup_pt_zero			
xtline markup_pt_zero, recast(line) tlabel(, labsize(medium) angle(vertical))			
gr export line_markup_pt_zero_`r'.pdf,replace			
			
histogram markup_pt_zero, kdensity kdenopts(lcolor(black) lwidth(thick) lpattern(solid) connect(direct))			
...
by size: gen delta_markup = (markup_pt_zero - l.markup_pt_zero) / l.markup_pt_zero	*100			
			
save acfest_data_pt_zero_`r',replace			
}	
}		
			
use acfest_data_pt_zero_1,clear			
			
forvalues r =  2/18{			
 append using acfest_data_pt_zero_`r'.dta			
 }			
 egen grp = group (year sec)			
label var grp group_variable			
sort grp			
save pt_markup_zero,replace			
save "C:\Users\Ege\Dropbox\WiSo 20-21\Data\Zero\HHI\Pt\pt_markup_zero.dta",replace	
